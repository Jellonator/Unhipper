...
impl HipData{
	...
	pub fn parse(data: &[u8]) -> Result<HipData, ()> {
		...
		let header_data = match util::load_chunk(&data, b"PACK", offset) {
			Ok(o) => {
				offset = o.next;
				match header::HeaderData::parse(&data[o.offset..o.next]) {
					Ok(data) => data,
					Err(_) => {return Err(());}
				}
			},
		...
		let directory_data = match util::load_chunk(&data, b"DICT", offset) {
			Ok(o) => {
				offset = o.next;
				directory::DirectoryData::parse(&data[o.offset..o.next])
			},
			Err(err) => {
				println!("{}", err);
				return Err(());
		...
		match f {
			Ok(ref mut filehandle) => {
				match filehandle.read_to_end(&mut data){
					Ok(_) => match HipData::parse(data.as_ref()) {
						Ok(val) => Some(val),
						Err(_) => None
					},
